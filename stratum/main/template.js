...
const Template = function (jobId, config, rpcData, placeholder) {
  ...
  // Template Variables
  this.target = _this.rpcData.target
    ? BigInt(`0x${_this.rpcData.target}`)
    : utils.bigIntFromBitsHex(_this.rpcData.bits);
  this.difficulty = parseFloat(
    (Algorithms.argon2d16000.diff / Number(_this.target)).toFixed(9)
  );
  this.previous = utils
    .reverseByteOrder(Buffer.from(_this.rpcData.previousblockhash, "hex"))
    .toString("hex");
  ...
  this.handleHeader = function (version, merkleRoot, nTime, nonce) {
    ...
    header.write(
      utils.reverseBuffer(merkleRoot).toString("hex"),
      (position += 4),
      32,
      "hex"
    );
  ...
  this.handleBlocks = function (header, coinbase) {
    return Buffer.concat([
      ...
      coinbase,
      Buffer.concat(
        _this.rpcData.transactions.map((tx) => Buffer.from(tx.data, "hex"))
      ),
    ]);
  };
